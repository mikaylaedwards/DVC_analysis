...
dfl %>% filter(room %in% c("Studio","1-Bedroom")) %>% 
  ...
  ggplot(aes(x=fct_reorder(season,avg_point),y=avg_point, group=room))+geom_col(aes(fill=room),position = "dodge")+geom_text(aes(label=round(avg_point,1)),
    position = position_dodge(0.9),
    vjust = 0)+scale_fill_manual(values = c("skyblue","grey"))
```
